...
export function renderSidebar(currentPath) {
  ...
  mount.innerHTML = groups.map((group) => {
    const links = CATALOG.filter((l) => l.track === group).map((l, idx) => {
      ...
      return `<a class="nav-link ${done} ${active}" href="${l.url}" ${active ? 'data-active-link' : ''}><span class="dot">${idx+1}</span><span>${l.label.replace(/^\d+\.\s/, '')}</span></a>`;
    }).join('');
  ...
  requestAnimationFrame(() => {
    const activeEl = mount.querySelector('[data-active-link]');
    if (activeEl) {
